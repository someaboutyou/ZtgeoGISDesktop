    ...
    public class SettingStore : ISettingStore, ITransientDependency
    {
        ...
        public virtual SettingInfo GetSettingOrNull(int? tenantId, long? userId, string name)
        {
            using (_unitOfWorkManager.Current.SetTenantId(tenantId))
            {
                using (_unitOfWorkManager.Current.DisableFilter(AbpDataFilters.MayHaveTenant))
                {
                    var setting = _settingRepository.FirstOrDefault(s => s.UserId == userId && s.Name == name && s.TenantId == tenantId);
                    var settingInfo=  setting.ToSettingInfo();
                    return settingInfo;//(_settingRepository.FirstOrDefault(s => s.UserId == userId && s.Name == name && s.TenantId == tenantId))
                   //.ToSettingInfo();
                }
            }
        }
